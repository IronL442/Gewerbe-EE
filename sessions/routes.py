...
def log_session():
    ...
    logger.debug(
        "🛠️ Field types -> student_name: %s, date: %s, start_time: %s, end_time: %s, session_topic: %s",
        type(student_name),
        type(date),
        type(start_time),
        type(end_time),
    ...
    if not all(
        ...
        logger.warning(
            "⚠️ Missing required fields: student_name=%s, date=%s, start_time=%s, end_time=%s, session_topic=%s, signature_data=%s",
            student_name,
            date,
            start_time,
            end_time,
